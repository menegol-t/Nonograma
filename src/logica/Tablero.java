...
public class Tablero {
	...
	public static int[][] generarReferenciaFilas(int[][] tableroConCasillasNegras)
	{
	    ...
	    int cantidadMaximaDeReferenciasPorFila = (tableroConCasillasNegras.length + 1) / 2;
	    
	    //Lista 5*3 donde guardo las referencias 
	    int [][] referenciasCalculadas = new int [tableroConCasillasNegras.length][cantidadMaximaDeReferenciasPorFila];
	        ...
	        for(int j = 0; j <= tableroConCasillasNegras[0].length - 1; j++)
	        {
	            ...
	            else 
	            {
	                ...
	                if(anteriorFueCeldaNegra)
	                {
	                    ...
	                    //Una vez guardado, como esta celda no es negra, corto el acumulador reiniciandolo. Si tengo la fila [1,1,0,1], esto se da cuando me paro en "0". En la linea anterior guardé el acumulador cuando estaba acumuladndo "2"
	                    acumuladorDeCeldasNegrasPorFila = 0;
