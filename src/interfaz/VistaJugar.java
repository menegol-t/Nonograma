...
public class VistaJugar extends JPanel
{
    ...
    private JPanel _panelReferenciasVerticales;
    private JPanel _panelOpciones;
    private JPanel _panelJuego;
    ...
    private int _tamanioTablero;

    private int[][] _referenciasFilas;
    private int[][] _referenciasColumnas; 

    private JButton _botonDePista;
    private JButton _botonLimpiarTablero;
    private JButton _botonDeVerificarResultado;

    private JButton[][] _casillas; 
    ...
    public VistaJugar(Frame frame)
    {
        ...
        _tamanioTablero = _controlador.getTamanioJuego();

        _referenciasFilas = _controlador.conseguirReferenciasFila();
        _referenciasColumnas = _controlador.conseguirReferenciasCol();
    ...
    {    
        ...
        _casillas = new JButton[_tamanioTablero][_tamanioTablero];

        for(int i = 0; i<_tamanioTablero; i++)
        {
            for(int j = 0; j<_tamanioTablero; j++)
            {    
                JButton casilla = generarCasilla();
                agregarAccionACasilla(casilla, i, j);
                _panelTablero.add(casilla);
                _casillas[i][j] = casilla;
            }
        }
    ...
    {
    	_panelOpciones = new JPanel(new GridLayout(10, 1, 5, 5));
        _panelOpciones.setBorder(new EmptyBorder(10, 10, 10, 11));
        _panelOpciones.setBackground(Color.DARK_GRAY);
        
        _panelJuego = new JPanel(new GridBagLayout());
        _panelJuego.setBackground(Color.DARK_GRAY);

//        panelJuego.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));
        
        add(_panelJuego, BorderLayout.CENTER);
        add(_panelOpciones, BorderLayout.EAST);
    ...
    {
        _panelTablero = new JPanel(new GridLayout(_tamanioTablero, _tamanioTablero));
        _panelTablero.setBackground(Color.LIGHT_GRAY);

        _panelReferenciasHorizontales = new JPanel(new GridLayout(_tamanioTablero, 1, 2, 2));
        ...
        _panelReferenciasVerticales = new JPanel(new GridLayout(1, _tamanioTablero, 2, 2));
        _panelReferenciasVerticales.setBackground(new Color(31, 43, 52, 200));
        _panelReferenciasVerticales.setBorder(BorderFactory.createEmptyBorder(5, 0, 5, 0));
        ...
        _panelJuego.add(panelVacio, diseño);

        diseño.gridx = 1; diseño.gridy = 0;
        diseño.weightx = 1.0; diseño.weighty = 0.0;
        diseño.fill = GridBagConstraints.BOTH;
        _panelJuego.add(_panelReferenciasVerticales, diseño);

        diseño.gridx = 0; diseño.gridy = 1;
        diseño.weightx = 0.0; diseño.weighty = 1.0;
        diseño.fill = GridBagConstraints.BOTH;
        _panelJuego.add(_panelReferenciasHorizontales, diseño);

        diseño.gridx = 1; diseño.gridy = 1;
        diseño.weightx = 1.0; diseño.weighty = 1.0;
        diseño.fill = GridBagConstraints.BOTH;
        _panelJuego.add(_panelTablero, diseño);
    }
    
    private void mostrarReferencias() 
    {
        _panelReferenciasHorizontales.setLayout(new GridLayout(_tamanioTablero, 1, 2, 2));
        _panelReferenciasHorizontales.setBackground(colorFondoReferencias);

        agregarReferenciasColumnas();

        _panelReferenciasVerticales.setLayout(new GridLayout(1, _tamanioTablero, 2, 2));
        _panelReferenciasVerticales.setBackground(colorFondoReferencias);
	...
	private void agregarReferenciasFilas() 
	{
		for (int i = 0; i < _tamanioTablero; i++) 
        {
            JPanel columna = new JPanel(new GridLayout(_referenciasColumnas[i].length, 1, 2, 2));
            columna.setBackground(colorFondoReferencias);
            columna.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5)); 
            for (int valor : _referenciasColumnas[i]) 
            {
            ...
            _panelReferenciasVerticales.add(columna);
        }
	}
	...
	private void agregarReferenciasColumnas() 
	{
		for (int i = 0; i < _tamanioTablero; i++) 
        {
            JPanel fila = new JPanel(new GridLayout(1, _referenciasFilas[i].length, 2, 2));
            fila.setBackground(colorFondoReferencias);
            fila.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5)); 
            
            for (int valor : _referenciasFilas[i]) 
            {
                JLabel referencia = new JLabel(valor == 0 ? "" : String.valueOf(valor), SwingConstants.CENTER);
                referencia.setFont(FUENTE_REFERENCIAS);
                referencia.setForeground(Color.WHITE);
    ...
    {
        _botonDeVerificarResultado = new JButton("Verificar Resultado");
        _botonDeVerificarResultado.setBorder(new EmptyBorder(0, 10, 0, 10));
        _panelOpciones.add(_botonDeVerificarResultado);
    }

    private void configurarBotonLimpiarTablero() 
    {
        _botonLimpiarTablero = new JButton("Limpiar tablero");
        _botonLimpiarTablero.setBorder(new EmptyBorder(0, 10, 0, 10));
        _panelOpciones.add(_botonLimpiarTablero);
        _botonLimpiarTablero.setFont(FUENTE_BOTONES);
    }

    private void limpiarTablero() 
    {
        if (_casillas == null) return;

        for (int i = 0; i < _tamanioTablero; i++) 
        {
            for (int j = 0; j < _tamanioTablero; j++) 
            {
                JButton celda = _casillas[i][j];
    ...
    {
        _botonDePista.addMouseListener(new MouseAdapter() 
        {
            @Override
            public void mouseClicked(MouseEvent evento) {
                JOptionPane.showMessageDialog(_botonDePista, "Toma una pista 🧩");
            }
        });

        _botonLimpiarTablero.addActionListener(evento -> limpiarTablero());

        _botonDeVerificarResultado.addMouseListener(new MouseAdapter() 
        {
            @Override
            public void mouseClicked(MouseEvent evento) 
            {
    ...
    {
        _botonDePista = new JButton("Dame Una Pista");
        _botonDePista.setBorder(new EmptyBorder(0, 10, 0, 10));
        _panelOpciones.add(_botonDePista);
        _botonDePista.setFont(FUENTE_BOTONES);
    }
}
